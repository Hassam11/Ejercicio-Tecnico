  ...
  try {
    ...
    const data = await res.json();
        ...
        data.Productos.forEach((element) => {
            ...
            div.innerHTML = `
                      <div class="card-body" data-categorias=${
                        element.category.id
                      } data-nombre=${element.name}>
                          <img  class="card-img-top img-productos" src=${
                            element.url_image
                          } alt=${element.url_image}  width="346px" height="465px" />
                          <h3 class="card-title text-center">${element.name}</h3>
                          <div class="precio">
                              <span class="fs-3 text-danger" class="card-text">$${element.price.toFixed(
                                2
                              )}</span>
